-- PostgreSQL equivalent of 0007_production_hardening.sql.
...
--> statement-breakpoint
UPDATE "settings" SET "value_json" = '"Povratne staklene flaše"', "updated_at" = to_char(CURRENT_TIMESTAMP AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS') WHERE "key" = 'trustItemTwo';
--> statement-breakpoint
UPDATE "settings" SET "value_json" = '"Tvrdnje o kvalitetu objavljujemo uz dokumentaciju"', "updated_at" = to_char(CURRENT_TIMESTAMP AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS') WHERE "key" = 'trustItemThree';

-- Add references after every table in this migration exists.
ALTER TABLE "subscription_mutation_versions" ADD FOREIGN KEY ("subscription_id") REFERENCES "subscriptions"("id") ON UPDATE no action ON DELETE no action;
