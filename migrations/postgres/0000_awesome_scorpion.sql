-- PostgreSQL equivalent of 0000_awesome_scorpion.sql.
...
CREATE TABLE "webhook_events" (
	...
	"payload_json" text NOT NULL,
	"received_at" text DEFAULT (to_char(CURRENT_TIMESTAMP AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS')) NOT NULL,
	"processed_at" text
);
--> statement-breakpoint
CREATE UNIQUE INDEX "webhook_events_provider_id_unique" ON "webhook_events" ("provider","provider_event_id");
--> statement-breakpoint
CREATE FUNCTION "audit_log_no_update_guard"() RETURNS trigger LANGUAGE plpgsql AS $guard$
BEGIN
  RAISE EXCEPTION 'audit_log is append-only' USING ERRCODE = '23514';
END;
$guard$;
CREATE TRIGGER "audit_log_no_update" BEFORE UPDATE ON "audit_log"
FOR EACH ROW EXECUTE FUNCTION "audit_log_no_update_guard"();
--> statement-breakpoint
CREATE FUNCTION "audit_log_no_delete_guard"() RETURNS trigger LANGUAGE plpgsql AS $guard$
BEGIN
  RAISE EXCEPTION 'audit_log is append-only' USING ERRCODE = '23514';
END;
$guard$;
CREATE TRIGGER "audit_log_no_delete" BEFORE DELETE ON "audit_log"
FOR EACH ROW EXECUTE FUNCTION "audit_log_no_delete_guard"();
--> statement-breakpoint
CREATE FUNCTION "outbox_no_delete_guard"() RETURNS trigger LANGUAGE plpgsql AS $guard$
BEGIN
...
ALTER TABLE "auth_tokens" ADD FOREIGN KEY ("customer_id") REFERENCES "customers"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "credits_ledger" ADD FOREIGN KEY ("customer_id") REFERENCES "customers"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "credits_ledger" ADD FOREIGN KEY ("subscription_id") REFERENCES "subscriptions"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "credits_ledger" ADD FOREIGN KEY ("order_id") REFERENCES "orders"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "delivery_items" ADD FOREIGN KEY ("delivery_order_id") REFERENCES "delivery_orders"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "delivery_orders" ADD FOREIGN KEY ("delivery_id") REFERENCES "deliveries"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "delivery_orders" ADD FOREIGN KEY ("source_order_id") REFERENCES "orders"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "delivery_orders" ADD FOREIGN KEY ("subscription_id") REFERENCES "subscriptions"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "delivery_orders" ADD FOREIGN KEY ("customer_id") REFERENCES "customers"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "next_delivery_addons" ADD FOREIGN KEY ("subscription_id") REFERENCES "subscriptions"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "next_delivery_addons" ADD FOREIGN KEY ("product_id") REFERENCES "products"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "order_items" ADD FOREIGN KEY ("order_id") REFERENCES "orders"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "order_items" ADD FOREIGN KEY ("product_id") REFERENCES "products"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "orders" ADD FOREIGN KEY ("customer_id") REFERENCES "customers"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "orders" ADD FOREIGN KEY ("subscription_id") REFERENCES "subscriptions"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "subscription_items" ADD FOREIGN KEY ("subscription_id") REFERENCES "subscriptions"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "subscription_items" ADD FOREIGN KEY ("product_id") REFERENCES "products"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "subscription_skips" ADD FOREIGN KEY ("subscription_id") REFERENCES "subscriptions"("id") ON UPDATE no action ON DELETE no action;
ALTER TABLE "subscriptions" ADD FOREIGN KEY ("customer_id") REFERENCES "customers"("id") ON UPDATE no action ON DELETE no action;
