...
export function AdminDashboard() {
  ...
  async function deleteProduct(product: Product) { if (!window.confirm(`Obrisati „${product.name}“? Ako ima porudžbine, biće bezbedno arhiviran.`)) return; start(); try { const result = await adminFetch<{ archived?: boolean }>(`/api/admin/products/${encodeURIComponent(product.id)}`, { method: "DELETE" }); await refresh(result.archived ? "Proizvod ima istoriju pa je arhiviran." : "Proizvod je obrisan."); } catch (e) { fail(e, "Proizvod nije obrisan."); } finally { setBusy(false); } }
