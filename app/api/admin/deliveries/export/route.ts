...
import { assertDomain, enumValue, withRoute } from "../../../../../server/domain";
...
export function GET(request: Request) {
  return withRoute(async () => {
    ...
    const format = enumValue(new URL(request.url).searchParams.get("format") ?? "csv", "format", ["csv", "xlsx"] as const);
    if (format === "xlsx") {
